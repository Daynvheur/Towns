	...
	private int getOffsetValue (String sValue, boolean firstValue) {
		...
		if (firstValue) {
			...
			try {
				...
				Log.log (Log.LEVEL.ERROR, Messages.getString ("LivingEntity.0") + sValue + "][" + e.toString () + "]", getClass ().toString ()); //$NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$
				return 0;
			}
		} else {
			// La Y
			try {
				...
				Log.log (Log.LEVEL.ERROR, Messages.getString ("LivingEntity.3") + sValue + "][" + e.toString () + "]", getClass ().toString ()); //$NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$
				return 0;
			}
		}
	}
			...
			if (losReady) {
				...
				if (fd != null && fd.getEntityID () != -1) {
					...
					if (lemi.getType () == TYPE_HERO && losReady) { // Redundant within this block (we're already gated on losReady) but kept to preserve original intent
						Hero hero = (Hero) this;
						if (hero.canChangeTask (true, true) && hero.getHeroData ().getHeroTask ().getTaskID () != Task.TASK_EQUIPING) {
							if (hasBetterItemInLOS (entityTypeInLOS (TYPE_HERO, TYPE_CITIZEN)) != null) {
								hero.getHeroData ().getHeroTask ().setTaskID (Task.TASK_EQUIPING);
								hero.getPath ().clear ();
							}
						}
					}
