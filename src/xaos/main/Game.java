...
public final class Game {
    ...
    public Game() {
        ...
        FPS_CAP = Towns.getPropertiesInt("FPS_CAP", FPS_CAP); //$NON-NLS-1$
        VSYNC = Towns.getProperty(MainProperties.VSYNC, VSYNC); //$NON-NLS-1$

        {
            // window size
            DisplayMode desktopMode = GLFWWindow.getDesktopDisplayMode();
            var desktopWidth = desktopMode.getWidth();
            var desktopHeight = desktopMode.getHeight();
        ...
        // Esto aquï¿½, despuï¿½s de cargar las texturas, que si no empieza el contador del SMP logo a bajar antes de tiempo
     ...
     *
     * @param campaignID ID de la campaï¿½a, null en caso de partida normal
     * @param missionID  ID de la misiï¿½n, null en caso de partida normal
     */
    ...
    private static void setupGame(boolean bNewGame, String sCampaignID, String sMissionID, boolean bForzedZip) {
        ...
        // Esto es un parche para solventar un bug. Antes no se borraban los ciudadanos de los job grupos al morir, y podrï¿½a estar corrupto
        ...
        // Paramos la mï¿½sica del main menï¿½ y arrancamos la mï¿½sica in-game
            ...
            if (sCampaignID != null && sMissionID != null) {
                ...
                if (missionData == null) {
                    if (bNewGame) {
                        ...
                        Game.exit();
                        // } else {
                        // // Cargando una partida y no encuentra la campaï¿½a, ponemos todo vacï¿½o y pacasa
...
//					MessagesPanel.addMessage (MessagesPanel.TYPE_SYSTEM, Messages.getString ("Game.5"), ColorGL.GREEN); //$NON-NLS-1$
    ...
    public static void generateBury(Cell[][][] cells) {
        ...
        while (bContinue) { // Nada de burying en el ï¿½ltimo nivel
            ...
            if (bd.getHash() != null) {
                ...
                if (iMaxZ == -1) {
                    ...
                    // Comprobaciï¿½n pq no estoy seguro que se pueda salir de rango
    ...
    /**
     * Obtiene eventos del mouse y llama al "panel" correspondiente segï¿½n donde sea el click
     */
    private void checkMouseEvents() {
        ...
        while (GLFWWindow.next()) {
            mouseButton = GLFWWindow.getEventButton();

            if (GLFWWindow.getEventButtonState()) {
                // Main menu
                if (getPanelMainMenu().isActive()) {
                ...
                if (mouseButton == 0) {
                    // Botï¿½n izquierdo pulsado, miramos a quï¿½ panel pertenece y llamamos a su funciï¿½n mousePressed (x, y) (con x e y relativas al panel)
                    // Primero miramos que no haya un contextmenu
                    if (getCurrentState() == STATE_SHOWING_CONTEXT_MENU) {
                        // Context menï¿½, miramos donde clica
                    ...
                    if (UIPanel.typingPanel != null) {
                        ...
                        // Botï¿½n derecho pulsado, cancelamos tarea (si hay)
            ...
            // Wheel
            if (GLFWWindow.getEventDWheel() > 0) {
                if (getPanelMainMenu().isActive()) {
                    continue;
                }
                world.keyPressed(KeyAdapter.KEY_NONE, KeyAdapter.FN_LEVEL_UP);
            } else if (GLFWWindow.getEventDWheel() < 0) {
                if (getPanelMainMenu().isActive()) {
                    continue;
                }
                world.keyPressed(KeyAdapter.KEY_NONE, KeyAdapter.FN_LEVEL_DOWN);
            }
        }

        GLFWWindow.resetMouseEvent();

        // Bordes (scroll de mouse)
        if (isMouseScrollON() && GLFWWindow.isInsideWindow()) {
            if (!getPanelMainMenu().isActive()) {
                final int BORDE = UIPanel.PIXELS_TO_BORDER; // Si se acerca X pixels al borde moveremos la cï¿½mara
                if (mouseX < BORDE) {
                    if (Game.isMouseScrollEarsON() || !UIPanel.isMouseCloseToOpenCloseProductionIcon(mouseX, mouseY)) {
                        if (getCurrentState() == STATE_SHOWING_CONTEXT_MENU) {
                            if (!(mouseX >= getCurrentContextMenu().getX() && mouseX < (getCurrentContextMenu().getX() + getCurrentContextMenu().getWidth()) && mouseY >= getCurrentContextMenu().getY() && mouseY < (getCurrentContextMenu().getY() + getCurrentContextMenu().getHeight()))) {
                                world.keyPressed(KeyAdapter.KEY_NONE, KeyAdapter.FN_LEFT);
                            }
                        } else {
                            world.keyPressed(KeyAdapter.KEY_NONE, KeyAdapter.FN_LEFT);
                        }
                    ...
                    // Miramos que no estï¿½ cerca del botï¿½n de abrir/cerrar el menu
                    if (Game.isMouseScrollEarsON() || !UIPanel.isMouseCloseToOpenCloseMenuIcon(mouseX, mouseY)) {
                        if (getCurrentState() == STATE_SHOWING_CONTEXT_MENU) {
                            if (!(mouseX >= getCurrentContextMenu().getX() && mouseX < (getCurrentContextMenu().getX() + getCurrentContextMenu().getWidth()) && mouseY >= getCurrentContextMenu().getY() && mouseY < (getCurrentContextMenu().getY() + getCurrentContextMenu().getHeight()))) {
                                world.keyPressed(KeyAdapter.KEY_NONE, KeyAdapter.FN_RIGHT);
                            }
                        } else {
                            world.keyPressed(KeyAdapter.KEY_NONE, KeyAdapter.FN_RIGHT);
                        }
                ...
                if (mouseY < BORDE) {
                    if (getCurrentState() == STATE_SHOWING_CONTEXT_MENU) {
                        if (!(mouseX >= getCurrentContextMenu().getX() && mouseX < (getCurrentContextMenu().getX() + getCurrentContextMenu().getWidth()) && mouseY >= getCurrentContextMenu().getY() && mouseY < (getCurrentContextMenu().getY() + getCurrentContextMenu().getHeight()))) {
                            world.keyPressed(KeyAdapter.KEY_NONE, KeyAdapter.FN_UP);
                        }
                    } else {
                        world.keyPressed(KeyAdapter.KEY_NONE, KeyAdapter.FN_UP);
                    }
                    ...
                    if (Game.isMouseScrollEarsON() || !UIPanel.isMouseCloseToOpenCloseBottomIcon(mouseX, mouseY)) {
                        if (getCurrentState() == STATE_SHOWING_CONTEXT_MENU) {
                            if (!(mouseX >= getCurrentContextMenu().getX() && mouseX < (getCurrentContextMenu().getX() + getCurrentContextMenu().getWidth()) && mouseY >= getCurrentContextMenu().getY() && mouseY < (getCurrentContextMenu().getY() + getCurrentContextMenu().getHeight()))) {
                                world.keyPressed(KeyAdapter.KEY_NONE, KeyAdapter.FN_DOWN);
                            }
                        } else {
                            world.keyPressed(KeyAdapter.KEY_NONE, KeyAdapter.FN_DOWN);
                        }
    ...
    private void checkKeyboardEvents() {
        while (KeyAdapter.next()) {
            if (KeyAdapter.getEventKeyState()) {
                // Tecla pulsada
                int iKEY = KeyAdapter.getEventKey();

                // Main menu
                if (getPanelMainMenu().isActive()) {
                    // Funciï¿½n
                    int iFN = KeyAdapter.getFN(iKEY);
                    if (iFN == KeyAdapter.FN_TOGGLE_FULLSCREEN) {
                        UtilsGL.toggleFullScreen();
                    } else {
                        // Ha tecleado algo en el main menu, quizï¿½ seteando el nombre del savegame
                ...
                if (UIPanel.typingPanel != null) {
                    ...
                    if (iKEY == KeyAdapter.KEY_ESCAPE && getCurrentState() == STATE_SHOWING_CONTEXT_MENU) {
                        // Back al contextMenu
                        ...
                        // } else if (iKEY == KeyAdapter.KEY_ESCAPE && getCurrentState() == STATE_SHOWING_INFO_PANEL) {
                        // closeInfoPanel();
                    } else {
                        // Funciï¿½n
                        int iFN = KeyAdapter.getFN(iKEY);

                        if (iFN == KeyAdapter.FN_SHOW_MISSION) {
                        ...
                        } else if (iFN == KeyAdapter.FN_SPEED_UP) {
                            if (World.SPEED < World.SPEED_MAX) {
                                World.addTurnsPerSecond();
                                MessagesPanel.addMessage(MessagesPanel.TYPE_SYSTEM, Messages.getString("Game.6") + World.SPEED, ColorGL.YELLOW); //$NON-NLS-1$
                            }
                        } else if (iFN == KeyAdapter.FN_TOGGLE_GRID) {
                            MainPanel.toggleGrid();
                        } else if (iFN == KeyAdapter.FN_TOGGLE_FLAT_MOUSE) {
                            MainPanel.toggleFlatMouse();
                        } else if (iFN == KeyAdapter.FN_TOGGLE_3D_MOUSE) {
                            MainPanel.toggle3DMouse();
                        } else if (iFN == KeyAdapter.FN_TOGGLE_HIDE_UI) {
                            MainPanel.toggleHideUI();
                        } else if (iFN == KeyAdapter.FN_TOGGLE_ITEM_BUILD_FACE) {
                            MainPanel.toggleItemBuildFace();
                        } else if (iFN == KeyAdapter.FN_SCREENSHOT) {
                            takeScreenshot = true;
                        } else {
                            // if (iFN == KeyAdapter.FN_UP || iFN == KeyAdapter.FN_DOWN || iFN == KeyAdapter.FN_LEFT || iFN == KeyAdapter.FN_RIGHT) {
    ...
    public void run() {
        ...
        // Hacemos sonar la mï¿½sica del main menï¿½
        UtilsAL.play(UtilsAL.SOURCE_MUSIC_MAINMENU);
//		int checkTrigger = (4 * FPS_INGAME); // Para que lo mire nada mï¿½s empezar, en el primer ciclo
//		boolean bVictory = false;

//		int iMissionCompletedWidth = (UtilFont.getWidth (Messages.getString ("Game.2"))) / 2; //$NON-NLS-1$
        while (!GLFWWindow.isCloseRequested()) {
...
*/
            ...
            //				GLFWWindow.setTitle ("FPS: " + fps); //$NON-NLS-1$
            // fps = 0;
            // lastFPS += 1000;
            // }
            // fps++;
            // Pongo este mï¿½todo pq dice la doc de LWJGL que actualiza los timers y tiene que llamarse lo mï¿½s posible
            // Sys.getTimerResolution() removed in LWJGL 3
...
*/
            ...
            if (getPanelMainMenu().isActive()) {
                GLFWWindow.sync(FPS_MAINMENU); // Para "capear" a 30 fps
            } else {
                if (FPS_CAP > 0) {
                    GLFWWindow.sync(FPS_CAP); // Para "capear" a FPS_CAP fps
                } else {
                    GLFWWindow.sync(FPS_INGAME); // Para "capear" a 30 fps
                }
            }
    ...
    private void handleResize() {
        if (GLFWWindow.wasResized() || GLFWWindow.isFullscreen() != displayFullscreen) {
            int iWidth = GLFWWindow.getWidth();
            int iHeight = GLFWWindow.getHeight();
            ...
            if (resetDisplayMode) {
                try {
                    GLFWWindow.setDisplayMode(iWidth, iHeight);
                } catch (Exception ex) {
            ...
            UtilsGL.initGLModes();
            UtilsGL.onResize(iWidth, iHeight, GLFWWindow.isFullscreen());
            Utils.saveOptions();

            displayFullscreen = GLFWWindow.isFullscreen();
        }
    }
}
